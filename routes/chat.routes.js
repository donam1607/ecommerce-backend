...
function isCouponRequest(message = '') {
  ...
  return /(ma giam gia|voucher|coupon|khuyen mai|uu dai|lay ma|xin ma|code giam)/.test(msg);
}
...
router.post('/', async (req, res) => {
  ...
  if (isGreetingMessage(message)) {
    return res.json({
      text: 'Dạ, ShopTech AI chào anh/chị ạ! Em có thể tư vấn sản phẩm, tìm laptop theo ngân sách, lấy mã giảm giá, tra đơn hàng hoặc giải đáp kỹ thuật. Anh/chị đang cần em hỗ trợ phần nào ạ?',
      products: []
    });
  }
